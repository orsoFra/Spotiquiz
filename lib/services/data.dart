...
class Data {
  ...
  Data.test({required API appi, required FirebaseFirestore ff}) {
    api = appi;
    db = ff;
  }
  ...
  Future storeResults(int score) async {
  ...
  Future<List> getLeaderboard() async {
    ...
    if (res.length < 3) for (int i = res.length; i < 3; i++) res.add({'totalScore': '', 'name': ''}); //if there are not enough players
