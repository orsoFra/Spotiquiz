  ...
  testWidgets('testUserImageProfile', (tester) async {
    await tester.runAsync(() async {
      await mockNetworkImagesFor(
          () async {
            when(() => mockAPI.getInfoUser(mockHttpClient)).thenAnswer((
                invocation) =>
                Future.value(
                  {
                    'imageUrl': '',
                    'display_name': 'flutterproject',
                    'email': 'flutterproject@gmail.com',
                    'product': 'premium'
                  },
                )
            );
            when(() => mocksD.getUserData()).thenAnswer((invocation) => Future.value(
              {
                'name': '',
                'numPerfectQuizzes': 'flutterproject',
                'numQuizzes': 'flutterproject@gmail.com',
                'totalScore': 'premium'
              }
            ));
